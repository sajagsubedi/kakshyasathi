...
import { CalendarClock, BookOpen, User, Clock, AlertCircle, DoorOpen, TrendingUp } from 'lucide-react';
import { PageHeader } from '@/components/shared/PageHeader';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Progress } from '@/components/ui/progress';
import { useStudentSchedule, useStudentProfile } from '@/hooks/useApi';
    ...
    return (
      <section>
        <PageHeader 
          ...
          description={profile?.className ? `${profile.className} · Section ${profile.sectionName}` : 'Holiday'}
        />
        <Card>
          <CardHeader>
  ...
  if (scheduleData.isWeeklyOff) {
    return (
      <section>
        <PageHeader 
          title="Today's Schedule" 
          description={profile?.className ? `${profile.className} · Section ${profile.sectionName}` : 'Weekly Off'}
        />
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-amber-600">
  ...
  return (
    <section className="space-y-6">
      <PageHeader 
        ...
        description={
          profile?.className 
            ? `${profile.className} · Section ${profile.sectionName} · ${scheduleData.date ? new Date(scheduleData.date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : ''}`
            : scheduleData.date ? new Date(scheduleData.date).toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' }) : ''
        }
      />
      ...
      <Card className="border-border bg-card/60 p-4">
        <div className="flex items-center justify-between text-xs mb-2">
          ...
          <span className="font-semibold text-foreground">{completedCount}/{schedule.length} periods completed</span>
        </div>
